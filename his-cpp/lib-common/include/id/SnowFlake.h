...
public:
	...
	/**
	 * 获取下一个ID
	 * @return uint64_t 返回计算出来的ID，返回0表示生成ID失败
	 */
	uint64_t nextId();
	/**
	 * 获取实例
	 * @param datacenterId 数据中心ID (0~31)，默认0
	 * @param machineId 工作机器ID(0~31)，默认基于物理MAC地址生成0~31的机器ID
	 */
	static SnowFlake* getInstance(int datacenterId = 0, int machineId = SnowFlake::generateMachineId());
	/**
	 * 基于物理MAC地址生成0~31的机器ID
	 * @return uint64_t 返回机器ID
	 */
	static uint64_t generateMachineId();
};
