...
NacosClient _ns(ServerInfo::getInstance().getNacosAddr(), ServerInfo::getInstance().getNacosNs(), ServerInfo::getInstance().getNacosUser(), ServerInfo::getInstance().getNacosPwd()); \
/** 读取配置数据节点 */ \
...
/** 获取redis相关配置 */ \
YamlHelper _yaml; \
string _host = _yaml.getString(&_config, "spring.data.redis.host"); \
string _port = _yaml.getString(&_config, "spring.data.redis.port"); \
string _password = _yaml.getString(&_config, "spring.data.redis.password"); \
/** 创建RedisClient对象 */ \
